...
OUTPUT_COLUMNS: Sequence[str] = FULL_TERMINAL_OUTPUT_COLUMN_ORDER
...
SAMPLE_CLASS_NAMES: Tuple[str, ...] = (
    ...
    "traj_corridor",  # 12 trajectory-corridor overlay: the bias tube the
                  ...
                  #    scripts/v764_append_full_terminal_corridor.py; not generated by
...
def _create_model_and_instance(
    ...
    writes to a shared ``OsdiModel`` buffer (AGENTS.md "Instance / Model
...
def _eval_single_point_with_reason(
    ...
    *,
    ...
    try:
        ...
        # OSDI currents point into the external terminals. Store three
        # independent surfaces in the solver's positive-leaving convention;
        # source is reconstructed by KCL at inference.
        out = {
            "i_d": -result["id"],
            "i_g": -result["ig"],
            "i_b": -result["ie"],
...
def _vds_zero_line_points(
    ...
    The caller passes the dataset's certified-envelope factor. 60 points per
    bin (20 Vg x 3 Vbs).
    """
...
def _reverse_vds_points(
    ...
    Without these samples the learned reverse-Vds terminal surfaces are
    unconstrained, leaving no reliable restoring current after a switching
    node rings past a supply rail.
...
def generate_one_bin(spec: BinSpec) -> Optional[Dict[str, np.ndarray]]:
    ...
    # The reverse_vds corridor teaches physical reverse conduction directly.
    for vg, vd, vbs in _vds_zero_line_points(
        spec.vdd, is_pmos, voltage_box_factor=spec.voltage_box_factor,
    ):
        _eval_and_keep(
    ...
    # centered on the switching band.
    ...
    # Overlay is VDD-relative (Vd ∈ [0.30·VDD, 0.70·VDD]).
    ...
    if spec.enable_inv_trip and spec.tech_name in (
        ...
        for vg, vd, vbs in _inv_trip_points(spec.vdd, is_pmos):
    ...
    *,
    ...
    for variant in variants:
        for L, NFIN in tech.get_geometry_combos(
            ...
            # V6.4.7 S9b: exclude NFIN<2. Project AGENTS.md fixes the
...
def generate_dataset(
    ...
    *,
    ...
    Returns:
        Dict with ``inputs`` (N,4), ``geometry`` (N,15), ``outputs`` (N,6),
        and ``metadata``.
    """
