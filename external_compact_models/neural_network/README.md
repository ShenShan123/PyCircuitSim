...
├── cli/train.py               unified training CLI
├── checkpoints/               generated weights and sidecars (ignored)
...
```bash
# From the PyCircuitSim repository root
python external_compact_models/bsim_cmg/scripts/generate_nn_data.py \
...
The root [environment setup section](../../README.md#0-set-up-the-environment)
owns the project-wide environment instructions. The package-local
`requirements.txt` is retained only as a historical note.
...
```bash
# Run training commands from the PyCircuitSim repository root.
# Train DirectNet v4-re (baseline MLP, zscore output norm, MAE + per-target LDS)
PYTHONPATH=external_compact_models conda run -n pycircuitsim \
...
The current measured verdicts and historical recipe outcomes live in the
project-level `docs/accuracy/` and `docs/CHANGELOG.md`.
