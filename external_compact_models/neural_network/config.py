...
#   parents[0] = neural_network/                  (NN_ROOT — package lives at the top of its dir)
#   parents[1] = external_compact_models/
#   parents[2] = <project root>                   (PROJECT_ROOT)
PROJECT_ROOT = Path(__file__).resolve().parents[2]
NN_ROOT = Path(__file__).resolve().parents[0]
import os as _os
CHECKPOINT_DIR = Path(_os.environ["BSIMAR_CHECKPOINT_DIR"]) if "BSIMAR_CHECKPOINT_DIR" in _os.environ else NN_ROOT / "checkpoints"
RESULTS_DIR = NN_ROOT / "results"
DATA_DIR = NN_ROOT / "data" / "datasets"

# ── Make pycmg importable ────────────────────────────────────────────────────
PYCMG_DIR = PROJECT_ROOT / "external_compact_models" / "bsim_cmg"
