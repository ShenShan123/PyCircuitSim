...
Builds a per-sample technology label array for ``universal_{device}.npz``
by enumerating every ``(tech, variant, L, NFIN)`` bin in the PyCMG
registry, reading its process parameters from the resolved modelcard,
and matching each sample's geometry row to a known fingerprint.
...
- TSMC12 and TSMC16 share identical ``(L, NFIN)`` grids and can only
  ...
  the lookup key is the full ``(NFIN, L, T, 12 proc params)`` tuple.

- The label array is cached next to the dataset as
  ``<dataset>_tech_labels.npy`` so subsequent LOO runs skip the scan.
"""
...
# Import PyCMG modelcard parser. ``bsimar.config`` has already appended
# the PyCMG root to ``sys.path`` on import, so this works without
# touching sys.path again.
