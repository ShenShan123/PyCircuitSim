...
Current shipping revision is **V6.4.5 (2026-05-30)** — the ring_osc + SRAM sprint, a **two-track** effort (plan `docs/plans/2026-05-28-directnet-v6.4.5-ro-sram.md`) merged into one ship. **Track A** (conservative, 5 phases, no-ship) ran a 32-training TSMC7 retrain and proved the RO gap is **not seed/recipe/retrain-addressable** (best feasible RO 9.05 % > baseline 8.98 %), and built the multi-circuit scorer (`scripts/eval_v6_4_5_candidate.py` + `scripts/v6_4_5_search.py`). **Track B** (unconstrained, B1–B9) then closed it: 8 levers KILL, but **B8 (differentiable-sim test-time fine-tuning)** swaps the **TSMC7 checkpoint to a TTFT fine-tune** that optimises the oscillation period *directly* → ring_osc TSMC7 **8.98 % → 0.32 %**, lifting the complex-circuit headline **9/16 → 10/16**. TSMC5/12/16 unchanged (V6.4.2 Phase-7a stock on TSMC5, V6.4.1 seed-42 on TSMC12/16). Inverter gate 8/8 held; extended harness 55/55 DC + 64/64 tran held. The shipped `tsmc7_dn_medium_{nmos,pmos}` is a plain DirectNet checkpoint (LoRA merged; sha256 nmos `26289484…`, pmos `3f94afe5…`; provenance `b8_ttft_tsmc7_*`; rollback `/tmp/v6_4_4_canonical_backup_20260529_094624/`). **Cost (documented):** the RO-only TTFT delta flattens the already-failing TSMC7 Miller opamp (gain 213→0) and worsens the SRAM force_ic diagnostic (resid 0.302→0.899) — neither is one of the 16 headline gates (no passing gate regressed); a multi-circuit-regularised TTFT is the V6.4.6 fix. V4 → V6.4.2 history in `docs/CHANGELOG.md` (unified V6.4.5 entry = Track A + Track B); final reports `results/v6_4_5_track_b/V6_4_5_track_b_final.md` (Track B, full B1–B9 roster) + `results/v6_4_5/V6_4_5_final.md` (Track A). **V6.4.4 (2026-05-28)** was the prior 9/16 ship. **SRAM force_ic stays open** (model-fidelity; ~10 levers across both tracks converge → V6.4.6 split-head — spectral-norm `id` head + softplus cap head).
