...
Current shipping revision is **V6.4.3** (V6.4.2 + BSIMAR code cleanup + V6.3.2-harness fixes + seed-lottery writeup + single-shot-stabilizer bake-off dead end, 2026-05-23..24). No checkpoint or solver change — V6.4.3 is hygiene-only on top of V6.4.2. V4/V5/V6/V6.1/V6.2/V6.2.1/V6.3/V6.3.1/V6.3.2/V6.4/V6.4.1/V6.4.2 history in `docs/CHANGELOG.md`; sprint detail in `docs/plans/2026-05-15-directnet-complex-circuits.md`.

> **Branch `feat/v6.4.3`, V6.4.3 (2026-05-23..24):** (1) deleted three stale BSIMAR files (`models/tsmc5_residual.py` + `training/tsmc5_residual_train.py`: V6 Tier M2 dead end; `data/analyze.py`: broken on import since v4-re removed `PROCESS_PARAM_NAMES`). (2) Documented the inverter-VTC seed lottery from `logs/v6_4_bestof/`: same data + pipeline + only `--seed` changing gives a 5–10× VTC-MaxErr ratio per tech because the MAE+LDS loss has a flat-minimum manifold over Id while the inverter trip's ~20× gain amplifies seed-dependent gm/gds residuals into Vout. Per-tech best NMOS/PMOS seeds: TSMC5 17/42, TSMC7 31337/42, TSMC12 123/123, TSMC16 42/123 — **no single seed wins all cells**. (3) Fixed three V6.3.2-harness defects merged into V6.4.1: env-var auto-set (OMP/MKL/OpenBLAS/NumExpr → 1) before any C-extension import; md5 checkpoint-drift guard in `nn_sweep.py` (raises if `bsimar/checkpoints/` files change mid-run); `verify_nn_dc_tran.py:get_available_checkpoints` fallback list extended to include `tsmc12_dn_*` / `tsmc16_dn_*`. Smoke-test `verify_nn_multi_tech_dc.py --tech TSMC5 --device nmos` → 7/7 PASS. (4) **Single-shot stabilizer bake-off — DEAD END**: 4 parallel agents in isolated worktrees tested EMA / SWA / Lookahead / EMA+Lookahead-combo on TSMC7 NMOS seed=42 (V6.4.1 worst case at 174.7 mV VTC MaxErr). All 4 *increased* MaxErr (+10.5% / +37.2% / +35.9% / +26.7%) while *lowering* NRMSE (5.0% → 3.60–4.43%) — clean confirmation that the loss is decoupled from inverter VTC by the trip's gain amplification. None promoted; trainer changes live only on orphan `worktree-agent-*` branches as dead-end evidence. See CHANGELOG "V6.4.3 — Single-shot stabilizer bake-off".

> **Branch `feat/v6.4.1`, V6.4.2 (2026-05-18..19):** continued the deferred plan phases. **Phase 5** (batched DirectNet forward+Jacobian, `d1fe87a`) — inverter bit-identical, opamp DC 3.4× faster; multi-device circuits diverge ~1e-8 from BLAS (metrics preserved; `NN_BATCHED_EVAL=0` recovers exact). **Phase 6** (NR convergence: LM damping + residual-norm gate + pseudo-transient DC, `35e9a16`) — non-regressing, but RO/SRAM gates stay open: those are model-accuracy gaps in the seed-42 checkpoints, not convergence failures. **Phase 4** (data overlays + sinh sampling, full best-of-N retrain) — **dead end**, reverted (`565de40`): no candidate beats V6.4.1, hard VTC↔transient tradeoff. **Phase 7** (soft physics constraints) — **dead end**, reverted: 7a monotone-in-Vg `id` residual loses to a plain retrain on every tech (best-of-4 VTC: TSMC5 121 vs 44, TSMC7 149 vs 100 mV); 7b spectral-norm gds is incoherent for a shared-trunk MLP (CLI rejects it). The bake-off's `stock` control arm beats V6.4.1 by ~90–110 mV VTC — recorded as a V6.4.3 best-of-N candidate, **not promoted** (sprint baseline pinned to V6.4.1 as-is). See CHANGELOG "V6.4.2".

> **Checkpoints unchanged — still V6.4.1 seed-42.** The on-disk `tsmc*_dn_medium_*` checkpoints were re-trained from a **single seed (42)**, not best-of-N. They pass all extended-harness gates (DC 55/55, VTC+tran 64/64) and the inverter gate (8/8) but **regressed inverter VTC MaxErr** vs the V6.4 best-of-N set (TSMC5 62→128, TSMC7 60→175, TSMC12 32→42, TSMC16 30→34 mV) — the seed lottery, as predicted. Phase 4 tried and failed to beat them with regenerated data. The shippable V6.4 best-of-N checkpoints are backed up at `/tmp/v6_4_checkpoints_backup_20260517/`. Restore them or run a full best-of-N before treating this branch as a model release. See CHANGELOG "V6.4.1"/"V6.4.2".
...
- **Open gate:** inverter VTC MaxErr ≤25 mV still unmet (V6.4 at 29.7–62.0; TSMC5/7 lag). Gain amplification at the trip multiplies the NN's Id residual ~20× into Vout. All three V6.4.2 accuracy levers are now exhausted: solver (Phase 6), data (Phase 4 dead end), network structure (Phase 7 dead end). The only remaining lever is a clean full best-of-N model release (the V6.4.3 candidate) or Phase 8 (per-axis trunk split). A solver-side gm-floor (Phase 2b) and Phase 7a monotonicity were both tried and **reverted** — see CHANGELOG "V6.4"/"V6.4.2".
...
- **Test infrastructure:** 3-level DC + transient suites (BSIM-CMG: 2+67+44 DC, 1+37+72 tran; NN: `verify_nn_dc_tran.py --tech TSMC5,TSMC7,TSMC12,TSMC16 --inverter-only` for the gate; per-tech routing via parser preempt cascade). Inverter metrics report: `scripts/eval_v6_3_1_inverter.py` → `results/v6_3_1_metrics_report/`. **V6.4 complex-circuit harness:** `tests/verify_complex_{ring_osc,opamp,sram_snm,switchcap}.py` + `tests/common/complex.py` (vs NGSPICE BSIM-CMG). V6.4.1-checkpoint result (2026-05-19): RO 2/4 (TSMC12/16 pass), opamp 2/4 (TSMC5/12 pass), SRAM butterfly-positive 4/4 but `force_ic` 0/4, switched-cap 1/4 — all open gates are the V6.4.1 seed-42 model-accuracy gap (TSMC5/7 laggards), not convergence failures.
