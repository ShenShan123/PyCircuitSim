...
19. **Analytical Vds correction** — `_MOSFETNNBase._apply_vds_correction()` enforces Id(Vds=0)=0 and Id=0 for reverse-direction Vds at inference time. Four-part correction (the order matters):
   - (a) **Rail-restoring extrapolation** when `|Vds| > VDD_train` (where `VDD_train = self._vdd_estimate`, derived from training norm stats): adds a *saturating-quadratic* Id ramp `id_cap · tanh(q/id_cap)` where `q = ½·g_max·overshoot²/x_ref` and `id_cap = g_max·x_ref` (with `g_max=1mS`, `x_ref=½·VDD_train`). The matching gds is `sech²(q/id_cap) · g_max·overshoot/x_ref`. At small overshoot this matches the pre-V5 quadratic Id + linear gds to second order (so converged in-distribution operating points are unchanged); at large overshoot Id saturates at `id_cap` (~0.2 mA at VDD=0.4V) and gds → 0, preventing the `1e150` runaway seen on TSMC5 BSIMAR-M VTC under deep NR overshoot. The plain `tanh(x/x_ref)` form is rejected — its slope at x=0 is g_max (not 0), which jumps gds discontinuously from the NN value to 1 mS at the boundary and drove TSMC12 inverter VTC from 13% PASS to 80%+ FAIL. Must run BEFORE the fast-path early-return.
