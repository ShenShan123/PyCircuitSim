...
Current shipping revision is **V6.4.5 (Track B, 2026-05-30, branch `feat/v6.4.5-track-b`)** — V6.4.4 mix with the **TSMC7 checkpoint swapped to the B8 differentiable-sim TTFT fine-tune**, which closes the TSMC7 ring-oscillator gate (period err 8.98 % → 0.32 %) and lifts the complex-circuit headline **9/16 → 10/16**. TSMC5/12/16 unchanged (V6.4.2 Phase-7a stock on TSMC5, V6.4.1 seed-42 on TSMC12/16). Inverter gate 8/8 held; extended harness 55/55 DC + 64/64 tran held. The shipped `tsmc7_dn_medium_{nmos,pmos}` is a plain DirectNet checkpoint (LoRA merged; sha256 nmos `26289484…`, pmos `3f94afe5…`; provenance `b8_ttft_tsmc7_*`; rollback `/tmp/v6_4_4_canonical_backup_20260529_094624/`). **Cost (documented):** the RO-only TTFT delta flattens the already-failing TSMC7 Miller opamp (gain 213→0) and worsens the SRAM force_ic diagnostic (resid 0.302→0.899) — neither is one of the 16 headline gates; a multi-circuit-regularised TTFT is the V6.4.6 fix. V4 → V6.4.2 history in `docs/CHANGELOG.md`; V6.4.5 plan in `docs/plans/2026-05-28-directnet-v6.4.5-ro-sram.md`; Track-B final report `results/v6_4_5_track_b/V6_4_5_track_b_final.md` (full B1–B9 roster). **V6.4.4 (2026-05-28)** was the prior 9/16 ship; **V6.4.5 Track A (2026-05-29)** was a no-ship iteration (five dead ends incl. a 32-training TSMC7 retrain, best RO 9.05 %) recorded in `results/v6_4_5/V6_4_5_final.md` — independently confirming the RO gap is unaddressable by retraining, which B8 closes by optimising the period directly. **SRAM force_ic stays open** (model-fidelity; six Track-B levers converge → V6.4.6 split-head). *Note: a concurrent Track-A session edited the main-checkout CLAUDE.md; reconcile at merge.*
