...
Current shipping revision is **V6.4.2** (V6.4.1 + DirectNet complex-circuits plan Phases 5–6 shipped, Phase 4 dead-ended, 2026-05-18). V4/V5/V6/V6.1/V6.2/V6.2.1/V6.3/V6.3.1/V6.3.2/V6.4/V6.4.1 history in `docs/CHANGELOG.md`; sprint detail in `docs/plans/2026-05-15-directnet-complex-circuits.md`.

> **Branch `feat/v6.4.1`, V6.4.2 (2026-05-18):** continued the deferred plan phases. **Phase 5** (batched DirectNet forward+Jacobian, `d1fe87a`) — inverter bit-identical, opamp DC 3.4× faster; multi-device circuits diverge ~1e-8 from BLAS (metrics preserved; `NN_BATCHED_EVAL=0` recovers exact). **Phase 6** (NR convergence: LM damping + residual-norm gate + pseudo-transient DC, `35e9a16`) — non-regressing, but RO/SRAM gates stay open: those are model-accuracy gaps in the seed-42 checkpoints, not convergence failures. **Phase 4** (data overlays + sinh sampling, full best-of-N retrain) — **dead end**, reverted (`565de40`): no candidate beats V6.4.1, hard VTC↔transient tradeoff. See CHANGELOG "V6.4.2".
