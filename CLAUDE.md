...
> **Branch `feat/v6.4.1` (2026-05-17):** the on-disk `tsmc*_dn_medium_*` checkpoints were re-trained from a **single seed (42)**, not best-of-N. They pass all extended-harness gates (DC 55/55, VTC+tran 64/64) but **regressed inverter VTC MaxErr** vs the V6.4 best-of-N set (TSMC5 62→128, TSMC7 60→175, TSMC12 32→42, TSMC16 30→34 mV) — the seed lottery, as predicted. The shippable V6.4 best-of-N checkpoints are backed up at `/tmp/v6_4_checkpoints_backup_20260517/`. Restore them or run a full best-of-N before treating V6.4.1 as a model release. See CHANGELOG "V6.4.1".
