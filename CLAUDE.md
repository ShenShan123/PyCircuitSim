...
- **tsmc5 switchcap ~11.7–12.1 %** over-conduction — **loss-compression-rooted**: tsmc5's small asinh `s_id`≈1.6e-5 puts the SC-relevant µA currents in the asinh-linear band where the loss gradient is compressed (S3 proved an EKV structural prior is neutral here — the analytic core fits the same compressed loss and over-conducts identically). Next attack: µA-band loss/normalization de-compression, or a transient/charge investigation of the SC gate (it's a sample-and-hold, not pure DC).
