...
> **V6.4.2 Phase-7a code now committed.** The `_MonotoneVgResidual` residual + `--monotonic` CLI flag in `bsimar/{cli/train,models/direct_net,training/trainer}.py` + `pycircuitsim/models/mosfet_directnet.py`, documented as shipped in V6.4.2 but never committed, are folded into the V6.4.4 commit. Plain (non-monotonic) checkpoints load through the same forward pass with `mono=None` — V6.4.4's shipped slots are all stock, no behaviour change.
