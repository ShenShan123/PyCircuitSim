...
19. **Analytical Vds correction** — `_MOSFETNNBase._apply_vds_correction()` enforces Id(Vds=0)=0 and Id=0 for reverse-direction Vds at inference time. Four-part correction (the order matters):
   - (a) **Rail-restoring extrapolation** when `|Vds| > VDD_train` (where `VDD_train = self._vdd_estimate`, derived from training norm stats): adds a *tanh-saturated* Id ramp `g_max·x_ref·tanh(overshoot/x_ref)` (with `g_max=1mS`, `x_ref=½·VDD_train`) and a matching *sech²* gds ramp `g_max·sech²(overshoot/x_ref)`. Id asymptotes to a hard cap of `g_max·x_ref` (≈ 0.2 mA at VDD = 0.4 V) and gds → 0 as overshoot grows, preventing the `1e150` runaway seen in TSMC5 BSIMAR-M VTC under deep NR overshoot. Must run BEFORE the fast-path early-return. (V5 Phase A: replaces the prior quadratic Id + linear gds ramp, which grew unbounded.)
