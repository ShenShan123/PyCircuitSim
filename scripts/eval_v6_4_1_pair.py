"""V6.4.1 Phase-4 best-of-N pair evaluator.
...
from the V6.4.1 Phase-4 backup at
``/tmp/v6_4_1_phase4_backup/checkpoints/``.

Identical in mechanism to ``scripts/eval_v6_4_pair.py`` but the restore
source is the V6.4.1 (seed-42) checkpoints — the Phase-4 comparison
baseline — not the V6.3.1 set.

Usage:
    python scripts/eval_v6_4_1_pair.py --tech TSMC5 \
        --nmos v6_4_1_p4_tsmc5_s42_nmos --pmos v6_4_1_p4_tsmc5_s42_pmos
"""
...
BACKUP_DIR = Path("/tmp/v6_4_1_phase4_backup/checkpoints")
...
def main() -> None:
    ...
    n_bk = len(list(BACKUP_DIR.glob("*_best.pt")))
    if n_bk < 8:
        sys.exit(f"V6.4.1 backup looks incomplete ({n_bk} .pt) — abort")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="v6_4_1_pair_") as tmp:
