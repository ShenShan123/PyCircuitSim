...
BACKUP_DIR = ROOT / "external_compact_models" / "bsimar" / "data" / "datasets_v2_backup"
...
def append_one(tech: str, dev: str) -> Dict:
    v2_path = DATA_DIR / f"{tech}_v2_{dev}.npz"
    frag_path = FRAG_DIR / f"{tech}_{dev}_corridor.npz"
    out_path = DATA_DIR / f"{tech}_v2cor_{dev}.npz"
...
def main() -> int:
    ...
    if not args.no_backup:
        ...
        for tech in techs:
            for dev in ("nmos", "pmos"):
                src = DATA_DIR / f"{tech}_v2_{dev}.npz"
    ...
    for tech in techs:
        for dev in ("nmos", "pmos"):
            t0 = time.time()
            rows.append(append_one(tech, dev))
