...
- **Success =** headline **>9/16** (rev 3 restatement: week 1 already banked 11/16, so campaign success means **net-new flips beyond the S8 state, target >11/16**) AND **SRAM force_ic improved (target 8/8)** AND — per ruling 4 — **derivative fidelity improved on promoted checkpoints** (autograd gm/gds/gmb NRMSE vs OSDI strictly better than control-v2; no cell regresses it), with promoted checkpoints trained on the unfiltered regen-v2 data (rulings 3/5), no protected gate regressed, and every killed arm/term recorded with the numbers that killed it.
