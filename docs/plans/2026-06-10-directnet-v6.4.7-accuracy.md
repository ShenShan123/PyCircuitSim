...
S1 pre-flight ✅ → S2 P0 ✅ → S3 R0.1 ✅ → S4 R0.2 ✅ → S5 R0.3 ✅ → S6 P1 ✅ → S7 P2 ✅ → S8 re-freeze baseline + scorer ✅ → S9 SWA/EMA infra ✅ → S9b regen-v2 + control-v2 ✅ → S10 P4 ✅ **KILLED** (Sobolev deriv arm: deriv fidelity ⟂ value-owned opamp) → **[rev-4 REORDER]** S12 P5 ✅ **KEEP** (trajectory-corridor: tsmc7 RO 8.28→2.9 %, per-tech mix 11→14/16) → S11 P3 ✅ **KILLED** (subthreshold-id arm: improves weak-inversion fidelity 3.5–5× + holds gates, but force_ic stays 0/14 and 6/7 cells COLLAPSE to the symmetric metastable point — force_ic is gain/NR-fixed-point owned, not value owned; → S17/P9) → S13 P8a → S14 P6 distillation → S15 P7 → S16 P8b (conditional) → **S17 P9 (force_ic fallback — now UNBLOCKED: S2+S7+S11 all failed to close force_ic)** → S18 Stage-B composition → S19 promotion. **S11b pivot (2026-06-15, user-directed):** the 2 open headline cells of the 14/16 mix are BOTH systematic model-fidelity limits — **tsmc5 SC over-conduction** (subthreshold barely moves it 12.16→11.70 %, corridor any-dose doesn't fix it; forward-conduction limit) and **tsmc7 opamp ~10–11 % gain over-prediction** (the deferred gentle-corridor W-sweep preserves-or-collapses the gain, no gentle correction; best `pivcor_w2_s7` opamp 10.78 % + RO/inv/SC pass = a better S19 tsmc7 candidate than the S12 corridor). **Resume point: S19 promotion at 14/16** (force_ic + tsmc5 SC + tsmc7 opamp as documented value-owned/forward-conduction known-issues) OR a scoped structural lever (architecture / P9 physics-core) if they are must-close; S13/S14/S15 are lower-value (S12 met the RO target). Gate `results/v6_4_7/S11b_pivot_open_cells.md`.
