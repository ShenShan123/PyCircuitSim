...
S1 ✅ → S2 P0 ✅ → S3 R0.1 ✅ → S4 R0.2 ✅ → S5 R0.3 ✅ → S6 P1 ✅ → S7 P2 ✅ → S8 re-freeze ✅ → S9 SWA/EMA ✅ → S9b regen-v2 + control-v2 ✅ → S10 P4 **KILL** → **[rev-4 reorder]** S12 P5 **KEEP** (→14/16 claimed) → S11 P3 **KILL** → S11b pivot (2 open cells = model-fidelity limits) → **S19 promotion ✅ SHIPPED at 13/16** (tsmc16 opamp flip retracted on authoritative-gate replication — see "S19 outcomes").

S13 P8a / S14 P6 / S15 P7 / S16 P8b / S17 P9 are **deferred**: S12 already met the RO target, and S11/S11b showed the remaining gaps (force_ic, tsmc5 SC, tsmc7 opamp, and now the retracted tsmc16 opamp) are value-surface / fixed-point / forward-conduction limits no cheap lever closes — S17/P9 is unblocked but its OFF-leakage premise is in tension with the S11 "force_ic is gain-owned" finding. **S19 promotion is COMPLETE — SHIPPED at 13/16** (the 4 gaps as documented known-issues; tsmc7=`pivcor_w2_s7`, tsmc16=`s12cor_w3_s31`, tsmc5+tsmc12=V6.4.4 baseline). Any further gain on the 4 known-issues needs a structural change (architecture / P9 physics-core).
