...
**STATUS (2026-06-16) — campaign converged at headline 14/16; recommend S19 promotion.** Per-step verdicts (detail in the "* outcomes" §§ below):
...
S1 ✅ → S2 P0 ✅ → S3 R0.1 ✅ → S4 R0.2 ✅ → S5 R0.3 ✅ → S6 P1 ✅ → S7 P2 ✅ → S8 re-freeze ✅ → S9 SWA/EMA ✅ → S9b regen-v2 + control-v2 ✅ → S10 P4 **KILL** → **[rev-4 reorder]** S12 P5 **KEEP** (→14/16) → S11 P3 **KILL** → S11b pivot (2 open cells = model-fidelity limits) → **S19 promotion at 14/16**.

S13 P8a / S14 P6 / S15 P7 / S16 P8b / S17 P9 are **deferred**: S12 already met the RO target, and S11/S11b showed the 3 remaining gaps (force_ic, tsmc5 SC, tsmc7 opamp) are value-surface / fixed-point / forward-conduction limits no cheap lever closes — S17/P9 is unblocked but its OFF-leakage premise is in tension with the S11 "force_ic is gain-owned" finding. **Resume point: S19 promotion at 14/16** (the 3 gaps as documented known-issues, swapping `pivcor_w2_s7` in for tsmc7), or a structural change (architecture / P9 physics-core) only if they are must-close.
...
- **Success (as planned) =** headline **>11/16** AND **force_ic 8/8**, no protected gate regressed, every killed arm recorded. **Achieved (2026-06-16): headline 14/16** (S12 +3) **— met; force_ic 8/8 — NOT met (still 0/8).** S11/S11b established the residual gaps are model-fidelity limits, so the honest ship is **14/16 with force_ic + tsmc5 SC + tsmc7 opamp as documented known-issues**; closing them needs a structural change, out of the cheap-lever scope. Promotion is on the circuit gates (16 cells + force_ic + inverter); derivative fidelity is reported, not promoted on (rev-4 ruling 6 — it anti-correlates with the value-owned opamp/RO).
