...
**Date:** 2026-05-28  •  **Status:** Track A COMPLETE (all 5 phases run; no-ship, 9/16) — 2026-05-29  •  **Branch:** `feat/v6.4.4`

> **Track A outcome (2026-05-29).** All five phases executed. Phases 1/2/4 = first-pass dead ends. **Phase 3** built the multi-circuit scorer (`scripts/eval_v6_4_5_candidate.py` + `scripts/v6_4_5_search.py`; `opamp_flat_flag` re-calibrated to `gain<10`; scorer accepts the V6.4.4 mix). **Phase 5** ran the full 16-seed stock + 8-seed mono TSMC7 retrain (32 fresh trainings) scored under that vector → **KILL**: best feasible RO 9.05 % > baseline 8.98 %, no candidate ≤5 %; the ~9 % RO error is a systematic model bias, not seed/recipe-addressable. No model shipped (9/16 = V6.4.4); canonical checkpoints sha-verified unchanged. Gate files: `results/v6_4_5/phase{1,2,3,4,5}_*.md`, final `results/v6_4_5/V6_4_5_final.md`. Closing TSMC7 RO is V6.4.6 split-head / Track B territory.
