...
> **Evidence boundary.** Every recipe result in this file is retained from the
> V7.3.0 campaign. V7.4.0 rebuilt only the clean DirectNet and BSIM-AR matrices
> on the new hardware, directly into the resolver-visible slots; it did not
> retrain or re-promote these recipe checkpoints. Treat the rows below as
> historical recipe evidence, not as a comparison against the V7.4.0 clean
> controls and not as the currently served artifact set.
...
|---|---|---|---|
| **`crit30f`** | `large` | `traj_corridor=3.0, inv_trip=2.0` | historical V7.3.0 production; corridor + inverter-trip anchor |
...
* **The pattern is systematic, not a one-off.** `crit30f@large` (historical V7.3.0 production)
