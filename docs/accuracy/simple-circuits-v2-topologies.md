...
| `diode_load` | OP, up/down supply and load-impedance DC | load-line intersection and continuation direction, with no source-pinned current |
| `mos_ratio_reference` | high-impedance `diode_load` supply DC | MOS-referenced voltage and line sensitivity without duplicating topology |
...
| `diffpair_active_load` | NMOS/PMOS OP and AC | role-sized input pairs, active mirror loads, internal bias nodes, gain, and bandwidth |
...
| `bias_tree_fanout` | 3/5/9/17-device OP | isolate coupled-network scale while holding the generated-bias question fixed |
