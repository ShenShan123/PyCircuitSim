#!/usr/bin/env python3
"""Parametric sweep driver — ringosc (DirectNet LEVEL=73 vs NGSPICE BSIM-CMG).
...
2=could-not-characterize). The single-point verify_complex_ringosc.py ship gate is
untouched.

Always CPU-pin:  CUDA_VISIBLE_DEVICES="" OMP_NUM_THREADS=1 MKL_NUM_THREADS=1

Usage:
    conda run -n pycircuitsim python tests/simple_circuits/verify_complex_ringosc_sweep.py
    conda run -n pycircuitsim python tests/simple_circuits/verify_complex_ringosc_sweep.py --tech TSMC16 --dimension vt_asym
"""
