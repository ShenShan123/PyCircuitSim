...
Provides technology profiles, test configuration, NGSPICE/PyCircuitSim runners,
...
# ---------------------------------------------------------------------------
# Technology definitions (5 technologies × multiple VT flavors)
# ---------------------------------------------------------------------------
ALL_TECHS: Dict[str, TechProfile] = {
    ...
    "TSMC5": TechProfile(
        ...
        default_nfin=2, default_vt="lvt",  # SVT pch has PDIBL2_i<0 at low NFIN
        vt_pairs=[
...
def plot_summary_bar(results: List[Dict[str, Any]], save_path: Path,
    ...
    ax.axhline(y=NRMSE_THRESHOLD * 100, color="red", lw=1.5, ls="--",
...
def run_test_suite(
    ...
    save_summary_csv(all_results, results_dir / "summary.csv")
    plot_summary_bar(all_results, results_dir / "summary.png", title=title)
