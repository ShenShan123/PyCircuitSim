...
from tests.common.simple_circuit_catalog import (
    AnalysisSpec, SIMPLE_V1, SIMPLE_V2, cases,
)
...
SIMPLE_V2_RENDER_SHA256_PATH = (
    Path(__file__).resolve().parent / "frozen_simple_v2_renders.py"
)


def simple_v2_render_digests() -> dict[str, str]:
    """SHA-256 of every nominal simple-v2 candidate and reference deck.

    Regenerate the frozen module only as a deliberate change, then review its
    diff in the same commit as the template or harness edit:

        python -c "import tests.test_circuit_harness_contracts as t; \
            t.write_simple_v2_render_freeze()"
    """
...
def write_simple_v2_render_freeze() -> None:
    """Rewrite ``tests/frozen_simple_v2_renders.py`` from the current renders."""
    digests = simple_v2_render_digests()
    lines = [
    ...
    for key in sorted(digests):
        lines.append(f'    "{key}":\n        "{digests[key]}",')
    lines.append("}")
    SIMPLE_V2_RENDER_SHA256_PATH.write_text("\n".join(lines) + "\n")


def test_simple_v2_rendered_decks_are_frozen() -> None:
    """Every nominal simple-v2 diagnostic deck must remain byte-identical.

    The 30 held-out cases carry a denominator so campaign passes can be
    compared; a template edit that changed what one of them measures used to
    fire no test (V7.7.2 audit A5).  The per-key map lives in
    ``tests/frozen_simple_v2_renders.py`` so a mismatch names the deck.
    """
    from tests.frozen_simple_v2_renders import SIMPLE_V2_RENDER_SHA256

    observed = simple_v2_render_digests()
    changed = sorted(
        key for key in SIMPLE_V2_RENDER_SHA256
        if observed.get(key) != SIMPLE_V2_RENDER_SHA256[key]
    )
    added = sorted(set(observed) - set(SIMPLE_V2_RENDER_SHA256))
    assert not changed and not added, (
        f"simple-v2 renders drifted: changed={changed[:8]} added={added[:8]}"
    )
    assert observed == SIMPLE_V2_RENDER_SHA256
...
def _lifted_source_marker(analysis: str, status: str = "pass") -> str:
    return GateResult(
        case_id="nn_lifted_source_dc",
        tech="TSMC12",
        corner="nominal",
        analysis=analysis,
        role="qualification",
        status=status,
        metrics={"mre_pct": 1.0, "r2": 0.999, "nrmse_pct": 2.0,
                 "max_err": 1e-6},
        domain={"vs0_v": 0.08, "vs0_fraction": 0.1},
        model_family="DirectNet-Full",
        model_level=75,
        checkpoint_pins={
            "nmos": "tsmc12_dnf_large_nmos",
...
def test_collector_requires_every_source_lift_from_the_canary(
    tmp_path: Path,
) -> None:
    ...
    assert is_verdict(entry)
    assert entry["metric"] == pytest.approx(2.0)
...
def _stimulus_render(case_id: str, analysis_name: str, corner: str) -> tuple[str, str]:
    case = next(item for item in cases() if item.case_id == case_id)
    analysis = next(item for item in case.analyses if item.name == analysis_name)
    return render_case_decks(
        case, analysis, BENCH["TSMC12"], CORNERS[corner],
        baked_lib=Path("/frozen/tsmc12.lib"), model_level=75,
    )


def _line_diff(before: str, after: str) -> list[tuple[str, str]]:
    return [
        (old, new) for old, new in zip(before.splitlines(), after.splitlines())
        if old != new
    ]


def test_slew_corner_scales_only_the_rendered_input_edges() -> None:
    """``slew_slow`` reaches a composite PULSE spec and touches nothing else."""
    nominal_c, nominal_r = _stimulus_render("inverter_energy", "switching", "nominal")
    slow_c, slow_r = _stimulus_render("inverter_energy", "switching", "slew_slow")

    candidate_diff = _line_diff(nominal_c, slow_c)
    assert len(candidate_diff) == 1
    old, new = candidate_diff[0]
    assert old.startswith("Vin") and "20p 20p" in old
    assert new == old.replace("20p 20p", "80p 80p")
    # The reference adapter receives the identical stress.
    reference_diff = _line_diff(nominal_r, slow_r)
    assert len(reference_diff) == 1
    assert reference_diff[0][0].startswith("Vin") and "20p 20p" in reference_diff[0][0]
    assert reference_diff[0][1] == reference_diff[0][0].replace("20p 20p", "80p 80p")


def test_load_corner_scales_literal_and_token_loads_exactly_once() -> None:
    nominal, _ = _stimulus_render("common_source_nn", "nmos_fixed", "nominal")
    heavy, heavy_reference = _stimulus_render("common_source_nn", "nmos_fixed", "load_heavy")
    assert _line_diff(nominal, heavy) == [("Cload out 0 10f", "Cload out 0 20f")]
    assert "Cload out 0 20f" in heavy_reference

    nominal, _ = _stimulus_render("inverter_energy", "switching", "nominal")
    heavy, _ = _stimulus_render("inverter_energy", "switching", "load_heavy")
    assert _line_diff(nominal, heavy) == [("Cload out 0 5f", "Cload out 0 10f")]


@pytest.mark.parametrize(
    ("case_id", "analysis_name", "corner", "applies"),
    (
        ("inverter_energy", "switching", "slew_slow", True),
        ("inverter_energy", "vtc", "slew_slow", False),
        ("inverter_energy", "switching", "load_heavy", True),
        ("common_source_nn", "nmos_fixed", "load_heavy", True),
        ("common_source_nn", "nmos_fixed", "slew_slow", False),
        ("current_mirror", "nmos", "load_heavy", False),
        ("ring_osc", "oscillation", "load_heavy", True),
        ("ring_osc", "oscillation", "slew_slow", False),
    ),
)
def test_stimulus_corners_apply_only_where_the_rendered_deck_changes(
    case_id: str, analysis_name: str, corner: str, applies: bool,
) -> None:
    case = next(item for item in cases() if item.case_id == case_id)
    analysis = next(item for item in case.analyses if item.name == analysis_name)
    assert analysis_applies_to_corner(
...
def test_stimulus_corners_leave_every_nominal_render_unchanged() -> None:
    """A scale of 1.0 must be a byte-level no-op for every existing corner."""
    for corner in CORNERS.values():
        if corner.name in {"slew_slow", "load_heavy"}:
            continue
        assert corner.slew_scale == 1.0 and corner.load_scale == 1.0
    from tests.common.simple_circuit_harness import _scale_spice_values

    assert _scale_spice_values("20p", 4.0) == "80p"
    assert _scale_spice_values("Cload out 0 10f", 2.0) == "Cload out 0 20f"
    assert _scale_spice_values("0.5f", 2.0) == "1f"
