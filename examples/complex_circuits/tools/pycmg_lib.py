...
# Vendored in-repo since V7.4.1.  Since V7.5.8 this tools/ tree is shared by
# every per-tech tree and sits at examples/complex_circuits/tools, so three
# parents up is the repository root (it was four when tools/ was duplicated
# inside each designs_tsmc*/).  PYCMG_DIR overrides.
PYCMG_DIR = Path(
    os.environ.get(
        "PYCMG_DIR",
...
#: ``examples/complex_circuits`` — the tree holding every ``designs_tsmc*``.
_BENCH_ROOT: Path = Path(__file__).resolve().parents[1]


def _resolve_tree() -> Path:
    """The per-tech design tree these tools operate on.

    Until V7.5.8 each ``designs_tsmc*/`` carried its own byte-identical copy of
    ``tools/``, so the tree was simply the tools directory's parent. One shared
    copy cannot know that, so the tree is resolved, in order, from:

      1. ``AG_TREE``  — an explicit path to a design tree;
      2. ``AG_TECH``  — a tech name, e.g. ``tsmc7`` (already the documented
         override for the tech itself, so it keeps meaning one thing);
      3. the working directory, if it is a design tree or inside one — which
         is how the documented workflows already run (`cd designs_tsmc7 &&
         python3 ../run_all.py`, `sh ../tools/pipeline.sh`).

    Raises if none resolve. The old code silently fell back to TSMC16 when the
    directory name did not match a known tech, which meant a mis-invocation
    produced a full set of confidently-wrong TSMC16 numbers under another
    tech's name — the loudest possible failure is the only safe one here.
    """
    env = os.environ.get("AG_TREE")
    if env:
        tree = Path(env).expanduser().resolve()
        if not tree.is_dir():
            raise SystemExit(f"AG_TREE is not a directory: {tree}")
        return tree

    tech = os.environ.get("AG_TECH")
    if tech:
        tree = _BENCH_ROOT / f"designs_{tech.lower()}"
        if not tree.is_dir():
            raise SystemExit(
                f"AG_TECH={tech} resolves to a missing tree: {tree}")
        return tree

    cwd = Path.cwd().resolve()
    for cand in (cwd, *cwd.parents):
        if cand.parent == _BENCH_ROOT and cand.name.startswith("designs_"):
            return cand

    raise SystemExit(
        "Cannot tell which design tree to use. The AnalogGym tools are shared "
        "across every tech now, so they need to be told:\n"
        f"  cd {_BENCH_ROOT}/designs_tsmc7 && python3 ../run_all.py\n"
        f"  AG_TECH=tsmc7 python3 {_BENCH_ROOT}/run_all.py\n"
        f"  AG_TREE=/path/to/designs_tsmc7 ...")


_ROOT: Path = _resolve_tree()

#: Public alias — every tool resolves its design tree through this one.
ROOT: Path = _ROOT
